...
graph TD
    ...
    %% === DÉTECTION MOTS-CLÉS ===
    INPUT_PROCESS --> KEYWORD_DETECT{🔍 Détection mots-clés<br/>file_keywords = ['crée', 'script'...]<br/>is_file_command = True}
    ...
    OLLAMA1 --> LLM1_RESPONSE[📤 Réponse LLM 1<br/>'{\"action\": \"creer\",<br/>\"fichier\": \"script.py\",<br/>\"chemin\": \"python\",<br/>\"instruction\": \"script Python générique\"}']
    
    %% === EXTRACTION JSON ===
    LLM1_RESPONSE --> JSON_EXTRACT[📄 Extract JSON<br/>regex: r'\{[\s\S]+\}'<br/>json.loads()]
    ...
    %% === GÉNÉRATION CODE ===
    CODE_GENERATION --> CODE_LLM_PROMPT[🤖 LLM Code Expert<br/>PROMPT: 'Tu es expert programmeur Python<br/>TÂCHE: Script complet pour...<br/>EXIGENCES: Fonctionnel, structure claire<br/>FORMAT: Commence par #!/usr/bin/env'<br/>Options: temp=0.1, predict=500]
    
    CODE_LLM_PROMPT --> OLLAMA3[(🤖 Ollama API<br/>Model: llama3.2)]
    OLLAMA3 --> CODE_RESPONSE[📤 Code généré<br/>#!/usr/bin/env python3<br/>Script utilitaire complet<br/>87 lignes, 2847 bytes]
    ...
    FILE_PATH --> FILE_WRITE[💾 Écriture fichier<br/>with open(file_path, 'w') as f:<br/>f.write(cleaned_code)]
    ...
    %% === FORMATAGE RÉSULTAT ===
    FILE_CREATED --> RESULT_FORMAT[📋 Formatage résultat<br/>'✅ Fichier créé: utilitaire_python.py<br/>📁 Emplacement: .../python/<br/>📝 Aperçu du code: #!/usr/bin...']
    ...
    %% === STOCKAGE ===
    FILE_WRITE --> BASE_DIR[(📁 Documents/AI_Assistant_Files/<br/>├── python/<br/>├── html/<br/>├── documents/<br/>└── recherches/)]
    
    %% === MÉTRIQUES ===
    END_FLOW --> METRICS[📊 Métriques<br/>Total: 3.2s<br/>LLM: 3.4s (80%)<br/>Fichier: 0.15s<br/>UI: 0.049s<br/>Tokens: 687]
