...
class CharacterWidget(tk.Frame):
    ...
    def __init__(self, parent, size: int = 80):
        ...
        self.canvas = tk.Canvas(
            ...
            cursor='hand2'  # Curseur en main pour indiquer qu'on peut cliquer
        )
        self.canvas.pack()
        
        # Bind pour clic sur le personnage
    ...
    def on_character_click(self, event):
        """Appelé quand on clique sur le personnage"""
        print("[CHARACTER] Clic sur le personnage - demande nouveau conseil")
        # Notifier le parent (main_window)
        parent_window = self.winfo_toplevel()
        if hasattr(parent_window, 'main_window_instance'):
            parent_window.main_window_instance._request_new_advice()
